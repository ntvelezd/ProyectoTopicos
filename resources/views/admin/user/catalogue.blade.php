                ...
                <div class="card-body">
                    <table style="width:100% ; border-spacing: 5px ">
                        ...
                        <tr>
                            ...
                            <td>{{ $user->getAdmin() }}</td>
                            <td>
                                <a href="edit/{{$user->getId()}}" class="btn btn-primary" role="button" aria-pressed="true">{{__('admin.edi')}}</a>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('admin.user.delete') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-primary" name="id" value="{{ ($user->getId()) }}">{{__('admin.del')}}</button>
